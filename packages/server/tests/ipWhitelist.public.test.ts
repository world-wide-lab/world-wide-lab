...
process.env.PUBLIC_IP_WHITELIST = "10.0.0.1,192.168.0.0/24";
const { default: app } = await import("../src/app");

const endpoint = request(app);

describe("PUBLIC_IP_WHITELIST", () => {
  it("should ignore requests to public endpoints", async () => {
    await expect(endpoint.get("/")).rejects.toThrow(

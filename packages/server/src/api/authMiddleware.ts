...
function requireAuthMiddleware(req: Request, res: Response, next: Function): void {
  ...
  if (!token) { noAuth(res, "Authentication via API Key required"); return; }

  // Verify authentication token
  // TODO: implement a more sophisticated authentication mechanism eventually
  if (!config.api.apiKey) {
    noAuth(res, "WWL is currently not set up to use an API Key. Please set the respective environment variable.");
    return;
  }

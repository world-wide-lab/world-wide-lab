 ...
 *                   within a session. Responses whose id already exists in
 *                   the same session are treated as duplicates and are not
 *                   stored again, so clients can safely re-send responses
 *                   they did not receive a confirmation for.
 *             required:
 *               - sessionId
 *               - name
 *               - payload
 ...
 *       '200':
 *         description: |
 *           Response created successfully. For duplicates, the id of the
 *           existing response is returned and duplicate is set to true.
 *       '400':
...
routerPublic.post(
  "/response",
  async (req: Request, res: Response, next: NextFunction) => {
    let responseParams: CreateResponseParams | undefined;
    try {
      ...
      if (
        ...
        }).catch(() => null)) as any as ResponseParams | null;

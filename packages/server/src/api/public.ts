 ...
 *                 type: integer
 *                 description: |
 *                   Optional id generated by the client, counting up from 0
 *                   within a session. When a response with the same
 *                   sessionId and clientResponseId already exists, it is
 *                   treated as a duplicate and no new response is stored.
 *                   This allows clients to safely re-send responses they did
 *                   not receive a confirmation for.
 *             required:
 *               - sessionId
 *               - name
 *               - payload
 ...
 *       '200':
 *         description: |
 *           Response created successfully. If the response already existed
 *           (same sessionId and clientResponseId), the id of the existing
 *           response is returned and duplicate is set to true.
 *       '400':
...
routerPublic.post(
  "/response",
  async (req: Request, res: Response, next: NextFunction) => {
    let responseParams: CreateResponseParams;
    try {
      responseParams = sanitizeNullBytes(responseSchema.validateSync(req.body));
    } catch (error) {
      next(error);
      return;
    }
    ...
    try {
      ...
      if (
        ...
        responseParams.clientResponseId !== undefined
      ) {
        // The client has sent this response before, but probably did not
        // receive our confirmation for it. Return the response which is
        // already stored instead of creating a duplicate.
        try {
          const existingResponse = (await sequelize.models.Response.findOne({
            where: {
              sessionId: responseParams.sessionId,
              clientResponseId: responseParams.clientResponseId,
            },
          })) as any as ResponseParams | null;
